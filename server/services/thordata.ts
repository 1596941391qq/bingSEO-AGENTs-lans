// ThorData API服务 - 仅用于获取Bing SERP数据
...
// ThorData SERP API调用
export async function getBingSerpData(keyword: string, targetLanguage: string = 'en') {
  const formData = new URLSearchParams();
  formData.append('engine', 'bing');
  ...
  try {
    console.log(`调用 ThorData API: ${keyword} (引擎: Bing)`);
    ...
    if (!response.ok && response.status === 401) {
      ...
      formDataWithoutAuth.append('q', keyword);
...
// 解析ThorData SERP响应
export function parseSerpResponse(data: any): {
  ...
  results.slice(0, 10).forEach((result: any) => {
    ...
    const snippet = result.description || result.result.snippet || '';
  ...
  if (results.length > 0) {
    ...
    } else if (url.includes('amazon.com') || url.includes('walmart.com')) {
