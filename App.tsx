...
const TEXT = {
  en: {
    ...
    serpEvidence: 'Bing Search Evidence',
    serpEvidenceDisclaimer: '* Data based on Bing Search grounding.',
  ...
  zh: {
    ...
    foundCount: '个高概率机会',
    serpEvidence: 'Bing 搜索证据',
    serpEvidenceDisclaimer: '* 数据基于 Bing 搜索实时分析。',
    showTransRef: '显示翻译对照',
        ...
        <div className="bg-white p-2 space-y-3 border-t border-slate-100">
           ...
           {keywords.map((kw) => (
             <div key={kw.id} className="border-b border-slate-100 last:border-0 pb-2 last:pb-0">
                ...
                <a 
                   href={`https://www.bing.com/search?q=${encodeURIComponent(kw.keyword)}`}
                   target="_blank"
  ...
  const runMiningLoop = async (startRound: number) => {
    ...
    while (!stopMiningRef.current) {
      ...
      try {
        ...
        addLog(`[Round ${currentRound}] Analyzing SERP probability (Bing)...`, 'api');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={reset}>
            ...
            <h1 className="text-xl font-bold text-slate-800 tracking-tight hidden sm:block">
              Bing SEO <span className="text-yellow-600">Agent</span>
            </h1>
          </div>
            ...
            <div className="text-center mb-10">
              ...
              <div className="flex w-full max-w-2xl mx-auto bg-white rounded-lg shadow-lg border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 transition-all">
                ...
                <button
                  ...
                  className="bg-yellow-600 hover:bg-yellow-700 text-white px-8 font-semibold transition-colors disabled:opacity-70"
                >
                  {t.btnStart}
                </button>
              </div>
              ...
              <div className="absolute inset-0 z-10 bg-white/90 backdrop-blur-sm rounded-xl flex items-center justify-center p-4 animate-fade-in">
                  <div className="bg-white rounded-xl shadow-2xl border border-green-200 p-8 max-w-md w-full text-center">
                      ...
                      <div className="flex flex-col gap-3">
                          <button 
                            onClick={goToResults}
                            className="w-full py-3 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors font-bold shadow-lg shadow-yellow-200"
                          >
                            {t.viewResults}
                          </button>
                          <button 
                          ...
                          >
                            ...
                            <td className="px-4 py-4 text-right">
                               <div className="flex items-center justify-end gap-3">
                                   ...
                                   <a 
                                      href={`https://www.bing.com/search?q=${encodeURIComponent(item.keyword)}`}
                                      target="_blank"
                                      rel="noopener noreferrer"
