...
export const analyzeRankingProbability = async (
  ...
  const analyzeSingleKeyword = async (keywordData: KeywordData): Promise<KeywordData> => {
    // Step 1: Fetch real Bing SERP results
    let serpData;
    let serpResults: any[] = [];
    ...
    const serpContext = serpResults.length > 0
      ? `\n\nTOP BING SEARCH RESULTS FOR REFERENCE (analyzing "${keywordData.keyword}"):\nNote: These are the TOP ranking results provided to you for competition analysis, NOT all search results.\n\n${serpResults.map((r, i) => `${i + 1}. Title: ${r.title}\n   URL: ${r.url}\n   Snippet: ${r.snippet}`).join('\n\n')}\n\nEstimated Total Results on Bing: ${serpResultCount > 0 ? serpResultCount.toLocaleString() : 'Unknown (Likely Many)'}\n\n⚠️ IMPORTANT: The results shown above are only the TOP-RANKING pages from Bing's first page. There may be thousands of other lower-ranking results not shown here. Use these top results to assess the QUALITY of competition you need to beat.`
...
TASK: Analyze the Bing SERP competition for the keyword: "${keywordData.keyword}".
${serpContext}
${serankingContext}
...
/**
 * Search Bing SERP for a keyword (wrapper for fetchSerpResults)
 * Returns SERP snippets in a simple array format
 */
export const searchBingSerp = async (
  keyword: string,
  targetLanguage: TargetLanguage
): Promise<SerpSnippet[]> => {
  try {
